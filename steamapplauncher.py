...
STEAM_FLATPAK_DIR = Path.home() / ".var/app/com.valvesoftware.Steam/Desktop"
STEAM_ICON_DIR = (
    Path.home()
    / ".var/app/com.valvesoftware.Steam/.local/share/icons/hicolor/256x256/apps/"
)
...
def rewrite_exec_line() -> None:
    for file in APPLAUNCHER_DIR.iterdir():
    ...
    for file in APPLAUNCHER_DIR.iterdir():
        ...
        with open(file, "w") as destination:
            for line in lines:
                if line.startswith("Icon="):
                    destination.write(
                        f"Icon={STEAM_ICON_DIR}/steam_icon_{app_id_number}.png\n"
                    )
                else:
...
def setup_logger() -> logging.Logger:
    ...
    log_file_handler = logging.FileHandler(
        Path.home() / ".cache/steamapplauncher.log", mode="a", encoding="UTF-8"
    )
